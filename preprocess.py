...
writer=tf.io.TFRecordWriter('reviews.tfrecord')
    ...
    for idx,item in review.iterrows():
        ...
        if not high:
            if stars==4 or stars==5:
                data_high={'text':_bytes_feature(bytes(item.text,'utf-8')),'stars':_float_feature((stars-1)*0.25)}
                high=True
        if not low:
            if stars==1 or stars==2:
                data_low={'text':_bytes_feature(bytes(item.text,'utf-8')),'stars':_float_feature((stars-1)*0.25)}
                low=True
        if high and low:
            break
